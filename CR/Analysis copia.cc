...
void Analysis::selection_ossf(int num_histo_kin,
    ...
    for(Int_t et=0;et< tree->GetEntries();et++){
      ...
      //cout<<"=======================    new event"<<endl;
        if (et % 100000 == 0 ) cout<<et<< ")   100000 loop"<< endl;
	...
	int counter_event_tight=0;
	test_ip=0;
        for (int i =0; i < _nLeptons; i++){
	  if (_flavors[i] == 0 && (!_passedMVA90[i] || !( _isolation_absolute[i]/_lPt[i] < 0.1))) counter_event_tight++;
	  if (_flavors[i] == 1 && (!_ismedium[i] || !( _isolation_absolute[i]/_lPt[i] < 0.1))) counter_event_tight++;
	  if(test_ip!=1){
	    if(TMath::Abs(_ipPV[i]) > 0.05 || TMath::Abs(_ipZPV[i]) > 0.1) test_ip=1;
	  }
        }
	if (counter_event_tight != 3) continue;
	...
	//==================== event pT claasification =====================
        ...
        if ((TMath::Abs(sum_3l_rec.M() - 91.1876) < 10) || sum_3l_rec.M() < 25) continue;
        */
	...
	*/
        ...
        if (event_clas == 1 ||event_clas == 2 ||event_clas == 3 ||event_clas == 4 || event_clas == 5 || event_clas == 6 ){
            ...
            histo_ossf_rec[1]-> Fill(m_T,w_factor);
            ...
            histo_ossf_rec[5]-> Fill(_n_bJets,w_factor);
        ...
        if (event_clas == 10 ||event_clas == 20 ||event_clas == 30 ||event_clas == 40 || event_clas == 50 || event_clas == 60){
            ...
            histo_n_ossf_rec[1]-> Fill(m_T,w_factor);
            ...
            histo_n_ossf_rec[5]-> Fill(_n_bJets,w_factor);
