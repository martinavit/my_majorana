...
void lett(string inputRootFile){
    ...
    ifstream wgn("txt_file/wg_ptcut_mmm_min.txt");
    if(!wgn){
    ...
    ifstream wzn("txt_file/wz_ptcut_mmm_min.txt");
    if(!wzn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    wzn.close();
    ifstream ttn("txt_file/tt_ptcut_mmm_min.txt");
    if(!ttn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    ttn.close();
    ifstream zgn("txt_file/zg_ptcut_mmm_min.txt");
    if(!zgn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    zgn.close();
    ifstream signal_50n("txt_file/signal_ptcut_50_mmm_min.txt");
    if(!signal_50n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    signal_50n.close();
    ifstream signal_20n("txt_file/signal_ptcut_20_mmm_min.txt");
    if(!signal_20n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    signal_20n.close();
    ifstream signal_10n("txt_file/signal_ptcut_10_mmm_min.txt");
    if(!signal_10n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    for (int i =0; i< npoint; i++){
      cout<<"pt > "<<i<<" signal: "<< signal_10_ossf[i]<<" - "<<signal_20_ossf[i]<<" - "<<signal_50_ossf[i]<<" back "<<back_value_ossf[i]<<endl;
    }
    ...
    double y_prova=0;
     for (int i =0; i< npoint; i++){
       g_pt_roc_50_ossf->GetPoint (i,x_prova,y_prova);
       cout<<"----> : "<<i<<") "<<x_prova<<"  vs "<< y_prova<<endl;
     }
    ...
    g_pt_tt_n_ossf = new TGraphErrors(npoint, x_point, back_value_ossf,  0, 0);
    ...
    c_pt_eff->Print("PLOT_PT/c_pt_eff_mmm_min.root");
    delete  c_pt_eff ;
    ...
    c_pt_eff_2d->Print("PLOT_PT/c_pt_eff_2d_mmm_min.root");
    delete  c_pt_eff_2d ;    
