...
void lett(string inputRootFile){
    ...
    ifstream wgn("txt_file/wg_ptcut_eee_min.txt");
    if(!wgn){
    ...
    ifstream wzn("txt_file/wz_ptcut_eee_min.txt");
    if(!wzn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    wzn.close();
    ifstream ttn("txt_file/tt_ptcut_eee_min.txt");
    if(!ttn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    ttn.close();
    ifstream zgn("txt_file/zg_ptcut_eee_min.txt");
    if(!zgn){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    zgn.close();
    ifstream signal_50n("txt_file/signal_ptcut_50_eee_min.txt");
    if(!signal_50n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    signal_50n.close();
    ifstream signal_20n("txt_file/signal_ptcut_20_eee_min.txt");
    if(!signal_20n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    signal_20n.close();
    ifstream signal_10n("txt_file/signal_ptcut_10_eee_min.txt");
    if(!signal_10n){
        cout<<"Il file non esiste "<<endl;
        return;
    }
    ...
    double new_den_wg_ossf[50];
    double new_num_wg_ossf[50];
    double  x_point_new[50];
     for (int i =0; i< 50; i++){
       x_point_new[i] = i*2;
       new_den_wg_ossf[i]= den_wg_ossf[i*2] + den_wg_ossf[(i*2)+1];
       new_num_wg_ossf[i]=num_wg_ossf[i*2] + num_wg_ossf[(i*2)+1];
        if (new_den_wg_ossf[i] == 0 )  wg_ossf[i] =0;
        else {
             wg_ossf[i]  = new_num_wg_ossf[i]/new_den_wg_ossf[i];
        }
     }
    ...
    for (int i =0; i< npoint; i++){
      //if (den_wg_ossf[i] == 0 ) wg_ossf[i]=0; den_wg_ossf[i];
      //else {
    ...
    g_pt_wg_ossf = new TGraphErrors(50, x_point_new, wg_ossf,  0, 0);
    ...
    c_pt_eff->Print("PLOT_PT/c_pt_eff_eee_min.root");
    delete  c_pt_eff ;
    ...
    c_pt_eff_2d->Print("PLOT_PT/c_pt_eff_2d_eee_min.root");
    delete  c_pt_eff_2d ;    
