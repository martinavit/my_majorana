...
Analysis::Analysis():TObject()

{
}
//_______________________________________________________ constructor_____
Analysis::Analysis(string FileNameTree_in):TObject()
...
//==================================================================
void Analysis::selection_ossf(int num_histo_kin,
			      int num_histo,
			      double xsection,
			      double luminosity,
  ...
  for(Int_t et=0;et< tree->GetEntries();et++){
    ...
    for(int i =0; i <_nLeptons; i++){
      if (_flavors[i] == 1 && _ismedium[i] && ( _isolation_absolute[i]/_lPt[i] < 0.1)) index_good_leptons.push_back(i);
      if (index_electron_too_close.size() !=0){
	if (_flavors[i] == 0 && _passedMVA90[i] && ( _isolation_absolute[i]/_lPt[i] < 0.1)){
	  if (i != index_electron_too_close.at(0) )	index_good_leptons.push_back(i);	
	}
      }
      if (index_electron_too_close.size() ==0){
	if (_flavors[i] == 0 && _passedMVA90[i] && ( _isolation_absolute[i]/_lPt[i] < 0.1)) index_good_leptons.push_back(i);
      }
    }
    ...
    //w_factor=1;
    //cout<<w_factor<<endl;
    //cout<<h_Counter->GetBinContent(1)<<endl;
    //cout<<h_Counter->GetEntries()<<endl;
    ...
    if (event_clas == 1 ||event_clas == 2 ||event_clas == 3 ||event_clas == 4 || event_clas == 5 || event_clas == 6 ){
      ...
      if (xsection > 116 && xsection < 118 ) { // zgamma
	zg_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
      if (xsection > 86 && xsection < 88 ) { // ttamma
	tt_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
      if (xsection > 488 && xsection < 490 ) { // wgamma
	wg_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
      if (xsection > 4 && xsection < 5 ) { // wzamma
	wz_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
      if (xsection > 71000 && xsection < 72000 ) { // wzamma
	dy1_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
      if (xsection > 5600 && xsection < 6000 ) { // wzamma
	dy2_2d_ossf->Fill(delta_ossf_R_min,delta_ossf_R_max);
      }
    ...
    if (event_clas == 10 ||event_clas == 20 ||event_clas == 30 ||event_clas == 40 || event_clas == 50 || event_clas == 60 ){
      ...
      if (xsection > 116 && xsection < 118 ) { // zgamma
	zg_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
      if (xsection > 86 && xsection < 88 ) { // ttamma
	tt_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
      if (xsection > 488 && xsection < 490 ) { // wgamma
	wg_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
      if (xsection > 4 && xsection < 5 ) { // wzamma
	wz_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
      if (xsection > 71000 && xsection < 72000 ) { // wzamma
	dy1_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
      if (xsection > 5600 && xsection < 6000 ) { // wzamma
	dy2_2d_n_ossf->Fill(delta_n_ossf_R_min,delta_n_ossf_R_max);
      }
  ...
  if (xsection > 116 && xsection < 118 ) { // zgamma
    std::ofstream zg_ptcut_min("codes_eff/txt_file/zg_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      zg_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 86 && xsection < 88 ) { // ttamma
    std::ofstream tt_ptcut_min("codes_eff/txt_file/tt_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      tt_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 488 && xsection < 490 ) { // wgamma
    std::ofstream wg_ptcut_min("codes_eff/txt_file/wg_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      wg_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 4 && xsection < 5 ) { // wzamma
    std::ofstream wz_ptcut_min("codes_eff/txt_file/wz_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      wz_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 71000 && xsection < 72000 ) { // wzamma
    std::ofstream dy1_ptcut_min("codes_eff/txt_file/dy1_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      dy1_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 5600 && xsection < 6000 ) { // wzamma
    std::ofstream dy2_ptcut_min("codes_eff/txt_file/dy2_ptcut_min.txt");
    for (int i =0; i < bin_num ; i++) {
      dy2_ptcut_min<<i<<" "<< numerator_min_ossf[i]<<" "<<numerator_min_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  ...
  if (xsection > 116 && xsection < 118 ) { // zgamma
    std::ofstream zg_ptcut_max("codes_eff/txt_file/zg_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      zg_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 86 && xsection < 88 ) { // ttamma
    std::ofstream tt_ptcut_max("codes_eff/txt_file/tt_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      tt_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 488 && xsection < 490 ) { // wgamma
    std::ofstream wg_ptcut_max("codes_eff/txt_file/wg_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      wg_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 4 && xsection < 5 ) { // wzamma
    std::ofstream wz_ptcut_max("codes_eff/txt_file/wz_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      wz_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 71000 && xsection < 72000 ) { // wzamma
    std::ofstream dy1_ptcut_max("codes_eff/txt_file/dy1_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      dy1_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
  if (xsection > 5600 && xsection < 6000 ) { // wzamma
    std::ofstream dy2_ptcut_max("codes_eff/txt_file/dy2_ptcut_max.txt");
    for (int i =0; i < bin_num ; i++) {
      dy2_ptcut_max<<i<<" "<< numerator_ossf[i]<<" "<<numerator_n_ossf[i]<<" "<<denominatore_ossf<<" "<<denominatore_n_ossf<<endl;
    }
  }
