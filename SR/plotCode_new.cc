...
void plotDataVSMC(TH1D* data, TH1D** bkg, const TString* names, const unsigned nHist, const TString& file, const bool ylog, const unsigned widthopt, const bool plotsig, TH1D** signal , const TString* signames, const unsigned nSig, const bool signorm ){
	...
	if(plotsig){
		...
		for(unsigned sig = 0; sig < nSig; ++sig){
			signal[sig]->SetLineColor(kRed);
			signal[sig]->SetMarkerColor(kRed);
    ...
    if(!ylog) data->SetMinimum(0.0000001);
    ...
    if(bkgTot->GetBinContent(bkgTot->GetMaximumBin()) > signal[0]->GetBinContent(signal[0]->GetMaximumBin()) ){
        if(!ylog) signal[0]->SetMaximum(bkgTot->GetBinContent(bkgTot->GetMaximumBin())*1.3);
        else signal[0]->SetMaximum(bkgTot->GetBinContent(bkgTot->GetMaximumBin())*2);
    }
    else{
        if(!ylog) signal[0]->SetMaximum(signal[0]->GetBinContent(signal[0]->GetMaximumBin())*1.3);
